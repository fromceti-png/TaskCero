...
canvas.addEventListener("mousedown", (e) => {
  ...
  const clickedOnObject = elements.some(el => {
...
canvas.addEventListener("mousemove", (e) => {
  if (!isDraggingBox || !selectionStart) return;
  const mouse = getMousePos(e);
  selectionBox = {
    x: Math.min(mouse.x, selectionStart.x),
    y: Math.min(mouse.y, selectionStart.y),
    w: Math.abs(mouse.x - selectionStart.x),
    h: Math.abs(mouse.y - selectionStart.y),
  };
  for (let el of elements) {
    const cx = el.x;
    const cy = el.y;
